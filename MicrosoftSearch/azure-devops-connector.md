...
Azure DevOps 连接器仅支持对此数据源或 **所有人都** **具有访问权限的用户** 可以看到的搜索权限。 如果选择 " **仅有权访问此数据源的人员**"，则根据 Azure DevOps 中组织、项目或区域路径级别的用户或组的权限对用户访问这些数据源的用户的搜索结果中将显示索引数据。 如果选择 " **每个人**"，则会在所有用户的搜索结果中显示索引的数据。
